...
<script>
    ...
    // Manage Variable Accept Reject 
    function updateManageAcceptRejectStatus(data) {
        var variable_request_info_id = data.id;
        var value = data.value;
        var id = variable_request_info_id.split("_")[1];

        var url = '{{ url('manage_variable_accept_reject_status') }}';
        $.ajaxSetup({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            }
    ...
    // MaageupdateAcceptRejectRemark
    $(document).on("keyup", ".updateManageAcceptRejectRemark", function() {
        var id = $(this).data('id');
        var value = $(this).val();

        var url = '{{ url('manage_variable_accept_reject_remark') }}';
        $.ajaxSetup({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            }
    ...
    // Fixed Assets Reject Remark
    $(document).on("keyup", ".fixedAssetsUpdateAcceptRejectRemark", function() {
        var id = $(this).data('id');
        var value = $(this).val();

        var url = '{{ url('fixed_assets_accept_reject_remark') }}';
        $.ajaxSetup({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            }
    ...
    // Fixed Assets Reject Remark
    function updateFixedAssetsAcceptRejectStatus(data) {
        var variable_request_info_id = data.id;
        var value = data.value;
        var id = variable_request_info_id.split("_")[1];

        var url = '{{ url('fixed_assets_accept_reject_status') }}';
        $.ajaxSetup({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            }
